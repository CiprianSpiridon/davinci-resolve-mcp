...
- exactly the 18 documented offline tool names are registered, each exactly
  ...
  connecting) tools registered by the server — 190 live + 18 offline = 208;
...
def test_offline_tool_names_are_unique_against_all_registered_tools(tools):
    ...
    # no offline name shadows/collides with any of the other 190 live
    # (Resolve-connecting) tools, and the full surface is exactly 208.
    ...
    assert len(live_names) == 190, (
        f"expected 190 live (Resolve-connecting) tools alongside the 18 "
        f"offline tools, found {len(live_names)}"
    )
    assert len(all_names) == 208, (
        f"expected 208 total registered tools (190 live + 18 offline), "
