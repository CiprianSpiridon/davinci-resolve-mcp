...
Auto-generated from `mcp.list_tools()` — **208 tools** (190 live + 18 offline) across 36 modules.
...
- **insert_generator** — Insert a generator (e.g
...
- **create_subtitles_from_audio** — Generate subtitles from audio on the current timeline using AI speech recognition
...
- **export_srt** — Transcribe an audio/video file locally and save the result as an SRT subtitle file
- **list_whisper_models** — List available local Whisper model names, as JSON, along with the default model
- **transcribe_and_add_subtitles** — Transcribe an audio/video file locally and add a marker per segment to the current Resolve timeline (so the transcript is visible/navigable in the tim
- **transcribe_audio** — Transcribe an audio/video file locally (mlx-whisper on Apple Silicon, openai-whisper elsewhere)
...
- **screenshot** — Take a screenshot so you can SEE the current state of the DaVinci Resolve UI

## Offline tools (operate on Resolve FILES + a local SQLite store — no Resolve needed)

### `tools/off_audio.py` (1)
- **offline_audio** — Offline audio loudness/level reads via the optional ffmpeg executable

### `tools/off_audio_plan.py` (1)
- **audio_plan** — Offline Fairlight track/stem planning + coverage/loudness analysis

### `tools/off_capabilities.py` (1)
- **capabilities** — Report OFFLINE/advanced tool-set capabilities as JSON

### `tools/off_color_trace.py` (1)
- **color_trace** — A better ColorTrace, offline: carry grades across a re-conform by clip identity

### `tools/off_conform.py` (1)
- **conform** — Offline conform/relink QC + lineage engine (OFFLINE/advanced tool set)

### `tools/off_deliverable.py` (1)
- **deliverable** — Deliverable QC / compliance: run a named compliance profile and report per-check pass/fail

### `tools/off_drp.py` (1)
- **drp** — Offline ``.drp`` (Resolve Project) reader/author/Media-Pool surgeon

### `tools/off_drt.py` (1)
- **drt** — Offline ``.drt``/``.drp`` timeline (SeqContainer) inspector/author/surgeon

### `tools/off_drx.py` (1)
- **drx** — Offline ``.drx`` (per-clip grade) inspector / codec / grading-catalog front door

### `tools/off_editorial.py` (1)
- **editorial** — Offline editorial integrity / changelist engine (OFFLINE/advanced tool set)

### `tools/off_fairlight.py` (1)
- **fairlight_plan** — Offline Fairlight bus-routing planner

### `tools/off_fusion.py` (1)
- **offline_fusion** — Read/inspect/edit a Fusion composition (.comp) file offline

### `tools/off_media.py` (1)
- **media_ingest** — Offline media-ingest / assistant-editor front end

### `tools/off_offline_ref.py` (1)
- **offline_ref** — Display-referred reference-frame extraction and shot-intent tagging

### `tools/off_pipeline.py` (1)
- **pipeline** — DB-as-truth pipeline orchestration for the OFFLINE/advanced tool set

### `tools/off_project_db.py` (1)
- **project_db** — DB-backed project operations for the OFFLINE/advanced tool set

### `tools/off_project_read.py` (1)
- **project_read** — Offline, read-only ``.drp``/``.drt`` project/timeline/clip inspector

### `tools/off_provenance.py` (1)
- **provenance** — Provenance / audit ledger for the OFFLINE/advanced tool set
